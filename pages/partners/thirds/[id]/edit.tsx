// pages/partners/thirds/[id]/edit.tsx
...
export default function EditThird(){
  ...
  const load = async ()=>{
    const { data: t } = await supabase.from('third_parties').select('*').eq('id', id).eq('kind','third').single()
    setRow(t||null)
  ...
  const save = async ()=>{
    try {
      ...
      alert('Guardado.')
      router.push(`/partners/thirds/${id}`)
    } catch (e:any) {
