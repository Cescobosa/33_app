...
export default function ArtistShow() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const filtered = Array.from(byCat.values())
        .filter((r: any) => {
  ...
  async function hardDelete() {
    ...
    try {
      // 1) borrar contratos del artista
      {
        const { error } = await supabase.from('artist_contracts').delete().eq('artist_id', artist.id);
        if (error) throw error;
      }
      // 2) borrar contratos y econ de terceros vinculados y los terceros
      ...
      for (const tp of tps || []) {
        const e1 = await supabase.from('third_party_contracts').delete().eq('third_party_id', tp.id);
        if (e1.error) throw e1.error;
        const e2 = await supabase.from('third_party_economics').delete().eq('third_party_id', tp.id);
        if (e2.error) throw e2.error;
      }
      const e3 = await supabase.from('third_parties').delete().eq('artist_id', artist.id);
      if (e3.error) throw e3.error;

      // 3) borrar economics y members
      {
        const e4 = await supabase.from('artist_economics').delete().eq('artist_id', artist.id);
        if (e4.error) throw e4.error;
      }
      {
        const e5 = await supabase.from('artist_members').delete().eq('artist_id', artist.id);
        if (e5.error) throw e5.error;
      }

      // 4) borrar artista
      {
        const e6 = await supabase.from('artists').delete().eq('id', artist.id);
    ...
    <Layout>
      ...
      <div className="module" style={{ display: 'flex', alignItems: 'center', gap: 16, justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button as="a" href={`/artists/${artist.id}/edit`} tone="neutral" icon="edit">
            Editar
          </Button>
          {artist.archived ? (
            <>
              <Button icon="restore" onClick={() => setArchived(false)}>Recuperar</Button>
              <Button tone="danger" icon="trash" onClick={hardDelete}>Borrar</Button>
            </>
          ) : (
            <Button tone="danger" icon="archive" onClick={() => setArchived(true)}>Archivar</Button>
          )}
        </div>
      </div>
      ...
      {members.map((m) => (
        <div key={m.id} className="card">
          <div className="row">
            ...
            {(m.tax_name || m.full_name) ? (
              <div style={{ flex: '1 1 260px' }}>
                <strong>Nombre fiscal / Empresa</strong>
