...
async function uploadAndSign(file: File) {
  ...
  const { error: upErr } = await supabase.storage.from(BUCKET_CONTRACTS).upload(name, file);
    ...
    <div>
      ...
      <div className="row" style={{ gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end' }}>
          <Button onClick={add}>+ Añadir contrato</Button>
        </div>
      </div>
      ...
      <div style={{ marginTop: 12 }}>
        {items.length === 0 ? (
          <small>No hay contratos subidos.</small>
        ) : (
          items.map((c) => (
